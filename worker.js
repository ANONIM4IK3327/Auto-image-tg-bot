...
function checkAccess(role, cmd) {
    ...
    const noneCmds = ["/start", "/help", "/ping", "/promptsuggest", "/img2txt"];

    if (role === "creator") return creatorCmds.includes(cmd);
    if (role === "tech") return techCmds.includes(cmd);
    if (role === "none") return noneCmds.includes(cmd);
    return false;
}
...
async function handleCommand(msg, env) {
    ...
    if (!text.startsWith("/")) {
        if (userRole === "none" && config.noneFeaturesEnabled && config.nonePromptSuggestEnabled) {
    ...
    switch (cmd) {
        ...
        case "/llmlist": {
            ...
            try {
                ...
                if (authRes.ok) {
                    ...
                    if (authData.data) {
                        const limit = authData.data.limit !== null ? `$${authData.data.limit}` : "Без лимита";
                        const usage = `$${authData.data.usage.toFixed(4)}`;
                        const isFree = authData.data.is_free_tier ? "Да" : "Нет";
                        info += `Потрачено: ${usage} / ${limit}\nБесплатный тир: ${isFree}\n`;
                        if (authData.data.rate_limit) {
                            info += `Лимит запросов: ${authData.data.rate_limit.requests} req / ${authData.data.rate_limit.interval}\n`;
                        }
                    }
                } else {
                    info += `Не удалось получить статус ключа (возможно, не поддерживается для этого ключа).\n`;
                }
        ...
        case "/img2txt": {
            ...
            if (userRole === "none") {
                if (!config.noneFeaturesEnabled || !config.noneImg2txtEnabled) {
                    return await tg.send(chatId, "🔒 img2txt отключен для роли none.");
                }
                ...
                if (cdUntil && Date.now() < cdUntil) {
                    ...
                    return await tg.send(chatId, `⏳ Для роли none действует кулдаун. Осталось ~${leftMin} мин.`);
                }
        ...
        case "/setrole": {
            ...
            if (role === "none") {
                config.roles[targetId] = "none";
                await tg.send(chatId, `✅ Пользователю <code>${targetId}</code> назначена роль: <b>none</b>.`);
            } else {
        ...
        case "/promptsuggest": {
            if (userRole !== "none") return await tg.send(chatId, "ℹ️ Эта команда предназначена для роли none.");
        ...
        case "/nonefeatures":
        case "/nonepromptsuggest":
        case "/noneimg2txt": {
            if (userRole !== "admin") return await tg.send(chatId, "🔒 Только Admin.");
            const val = (params[0] || "").toLowerCase();
            if (!["on", "off"].includes(val)) return await tg.send(chatId, `❌ ${cmd} <on|off>`);
            const flag = val === "on";
            if (cmd === "/nonefeatures") config.noneFeaturesEnabled = flag;
            if (cmd === "/nonepromptsuggest") config.nonePromptSuggestEnabled = flag;
            if (cmd === "/noneimg2txt") config.noneImg2txtEnabled = flag;
            await saveConfig(env, config);
            await tg.send(chatId, `✅ ${cmd}: ${flag ? "ВКЛ" : "ВЫКЛ"}`);
            break;
        }

        case "/setnonecooldown": {
            if (userRole !== "admin") return await tg.send(chatId, "🔒 Только Admin.");
            const val = parseInt(params[0], 10);
            if (!val || val < 60 || val > 86400) return await tg.send(chatId, "❌ /setnonecooldown <секунды 60-86400>");
            config.noneImg2txtCooldownSec = val;
            await saveConfig(env, config);
            await tg.send(chatId, `✅ Cooldown для none img2txt: ${val} сек.`);
            break;
        }
